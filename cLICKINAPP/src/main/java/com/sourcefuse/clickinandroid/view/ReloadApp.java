                ...
                if (mPartnerId != null) {
                    ...
                    if (partnerIndex != -1) {
                      ...
                      //  intent1.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
                        ...
                        //it means partner no more exists in relation list-so simply move the user to home screen
                        intent1.setClass(this, UserProfileView.class);
                           intent1.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
                 //       intent1.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        // intent.setFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT);
      ...
      //  int count=-1;
        for (GetrelationshipsBean temp : ModelManager.getInstance().getRelationManager().acceptedList) {
            ...
            if (temp.getPartner_id().equalsIgnoreCase(partnerId)) {
                return index; //if got matched, then return the count at location partner exists
            }
        }
        return -1;
    }
