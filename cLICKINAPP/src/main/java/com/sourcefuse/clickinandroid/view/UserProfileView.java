...
//                    Toast.LENGTH_SHORT).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      Log.e("Mixpanel id",ModelManager.getInstance().getAuthorizationManager().getPhoneNo());
        ...
        if(Utils.isEmptyString(Utils.deviceId))
            Utils.deviceId=Utils.getRegId(UserProfileView.this);
...
//start service first, so that it will not get destroyed when unbind.
    ...
    public void onStart() {
        super.onStart();
      //  MixpanelAPI mMixpanel = MixpanelAPI.getInstance(this, Constants.MIX_PANEL_TOKEN);
      //  MixpanelAPI.People people = MixpanelAPI.getInstance(this, Constants.MIX_PANEL_TOKEN).getPeople();
        if(!Utils.isEmptyString(Utils.deviceId))
            MixpanelAPI.getInstance(this, Constants.MIX_PANEL_TOKEN).getPeople().setPushRegistrationId(Utils.deviceId);
